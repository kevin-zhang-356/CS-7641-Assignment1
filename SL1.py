...
list2=[]
for i in range(1,95):
    clf = DecisionTreeClassifier(max_depth=10, min_samples_leaf = 4, min_samples_split= 9)
...
# clf = DecisionTreeClassifier(max_depth=5, min_samples_leaf = 4, min_samples_split= 9)
# clf = clf.fit(X_train, y_train)
# test_predict = clf.predict(X_test)
...
list2=[]
for i in range(1,95):
    clf = MLPClassifier(solver='sgd', alpha=1e-5, hidden_layer_sizes=(5,2), activation='logistic')
...
for i in range(1,95):
    ...
    X_train, X_test, y_train, y_test = train_test_split(Xn, y, random_state=100, test_size=1-i/100)
    clf.fit(X_train, y_train)
    train_predict = clf.predict(X_train)
    test_predict = clf.predict(X_test)
    list1.append(accuracy_score(y_train, train_predict))
...
#learning curve of KNN classifier with K=3
list1=[]
list2=[]
for i in range(1,95):
